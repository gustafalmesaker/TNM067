...
float divergence(vec2 coord){
    ...
    vec2 v1 = texture(inport, vec2(coord.x + pixelSize.x, coord.y)).xy;
    vec2 v2 = texture(inport, vec2(coord.x - pixelSize.x, coord.y)).xy;

    vec2 v3 = texture(inport, vec2(coord.x, coord.y + pixelSize.y)).xy;
    vec2 v4 = texture(inport, vec2(coord.x, coord.y - pixelSize.y)).xy;
